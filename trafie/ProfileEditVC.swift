  ...
  @IBAction func saveProfile(_ sender: AnyObject) {
    ...
    Alamofire.upload(
      multipartFormData: { mfd in
        if self._profileImageEdited, let imageData: Data = UIImageJPEGRepresentation(self.profileImage.image!, 1) {
          mfd.append(imageData, withName: "picture", fileName: "profile-picture.jpeg", mimeType: "image/jpeg")
        }
...
//            let _value : NSString = value as! NSString
          ...
          if value is NSDictionary {
            ...
            do {
              try mfd.append(JSONSerialization.data(withJSONObject: value, options: options), withName: key)
              print(value)
